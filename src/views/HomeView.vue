...
v-row.text-center#home
  v-col(cols="12")
    ...
    h1 {{ timeText }}
  v-col(cols="12")
    v-btn.mx-3(icon color="#59DEEB" v-if="status !== 1" @click="startTimer")
      v-icon mdi-play
    v-btn.mx-3(icon color="black" v-else @click="pauseTimer")
      v-icon mdi-pause
    v-btn.mx-3(icon color="#DE413E" v-if="current.length > 0" @click="finishTimer(true)")
